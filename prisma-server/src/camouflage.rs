...
use prisma_core::types::PROTOCOL_VERSION;
...
/// and random bytes (version unlikely to be exactly 0x01 with valid length).
pub fn looks_like_prisma_hello(bytes: &[u8]) -> bool {
    ...
    let version = bytes[2];
    frame_len >= MIN_CLIENT_HELLO_SIZE && version == PROTOCOL_VERSION
}
...
mod tests {
    ...
    fn test_prisma_hello_valid() {
        // frame_len=100 (0x0064), version=0x01
